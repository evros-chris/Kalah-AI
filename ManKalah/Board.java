...
public class Board extends Observable implements Cloneable
{
  ...
  public Board clone() throws CloneNotSupportedException { return new Board(this); }
  
  public int getNoOfHoles() { return this.holes; }
  ...
  public void addSeeds(Side side, int hole, int seeds) throws IllegalArgumentException {
    ...
    this.board[indexOfSide(side)][hole] = this.board[indexOfSide(side)][hole] + seeds;
    setChanged();
  }
  ...
  public void addSeedsOp(Side side, int hole, int seeds) throws IllegalArgumentException {
    ...
    this.board[1 - indexOfSide(side)][this.holes + 1 - hole] = this.board[1 - indexOfSide(side)][this.holes + 1 - hole] + seeds;
    setChanged();
  }
  
  public int getSeedsInStore(Side side) { return this.board[indexOfSide(side)][0]; }
  ...
  public void addSeedsToStore(Side side, int seeds) throws IllegalArgumentException {
    ...
    this.board[indexOfSide(side)][0] = this.board[indexOfSide(side)][0] + seeds;
    setChanged();
  }
  ...
  public String toString() {
    ...
    boardString.append(String.valueOf(this.board[0][0]) + "  --");
    for (int i = this.holes; i >= 1; i--)
      boardString.append("  " + this.board[0][i]); 
    boardString.append("\n");
    for (int i = 1; i <= this.holes; i++)
      boardString.append(String.valueOf(this.board[1][i]) + "  "); 
